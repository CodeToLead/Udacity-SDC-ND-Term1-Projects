...
from TransformPerspective import TransformPerspective
...
def process_single_image(file_name):
    ...
    lf = FindLane(calibrator, perspective_transformer)
...
def process_video(video_name):
    ...
    lf = FindLane(calibrator, perspective_transformer, n_frames=7)
...
if __name__ == "__main__":
    ...
    calibrator = CalibrateCamera(CAMERA_CAL_IMAGES_PATH, CAMERA_CAL_IMAGE_NAMES, CAMERA_CAL_PICKLE_NAME)

    # Create the perspective transform - one-off (assumption: the road is a flat plane)
    perspective_transformer = TransformPerspective()
